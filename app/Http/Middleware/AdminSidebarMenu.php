...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            // Modules goes here
            $bussines = Business::find(session('business.id'));
            // $super_admin = SuperAdmin::find(session('business.id'));
            //Manufacturing
            if ($bussines->manufacturing == 1) {
                if (auth()->user()->can('manufacturing.access_recipe') || auth()->user()->can('manufacturing.access_production')) {
                    $menu->url(action('\Modules\Manufacturing\Http\Controllers\RecipeController@index'), 'Manufacturing', ['icon' => 'fa fas fa-industry', 'active' => request()->segment(1) == 'manufacturing'])->order(21);
                }
            }
            //essential
            if ($bussines->essential) {
                $menu->url(
            ...
            //Repair
            if ($bussines->repair == 1) {
                if (auth()->user()->can('superadmin') || auth()->user()->can('repair.view') || auth()->user()->can('job_sheet.view_assigned') || auth()->user()->can('job_sheet.view_all')) {
                    $menu->url(action('\Modules\Repair\Http\Controllers\DashboardController@index'), '<span style="margin-left: 11px;">Repair</span>', ['icon' => 'fas fa-wrench', 'active' => request()->segment(1) == 'repair'])->order(24);
                }
            }

            //Product catelog
            if ($bussines->product_catelogue == 1) {
                $menu->url(action('\Modules\ProductCatalogue\Http\Controllers\ProductCatalogueController@generateQr'), '<span style="margin-left: 12px;">Product Catelogue</span>', ['icon' => 'fas fa-qrcode', 'active' => request()->segment(1) == 'repair'])->order(88);
            }

            //shop module
            if ($bussines->shop_module == 1) {
                $menu->url(action('\Modules\Shop\Http\Controllers\DashboardController@index'), '<span style="margin-left: 8px;">Shop</span>', ['icon' => 'fas fa-store-alt', 'active' => request()->segment(1) == 'repair'])->order(24);
            }

            if ($bussines->woocommerce == 1) {
                if ((auth()->user()->can('woocommerce.syc_categories') || auth()->user()->can('woocommerce.sync_products') || auth()->user()->can('woocommerce.sync_orders') || auth()->user()->can('woocommerce.map_tax_rates') || auth()->user()->can('woocommerce.access_woocommerce_api_settings'))) {
                    $menu->url(
            ...
            //accounting module
            if ($bussines->accounting == 1) {
                $menu->url(action('\Modules\Accounting\Http\Controllers\DashboardController@index'), '<span style="margin-left: 12px;">Accounting</span>', ['icon' => 'fas fa-book', 'active' => request()->segment(1) == 'repair'])->order(49);
            }
