...

{
    ...
    public function payrollGroupDatatable(Request $request)
    {
        ...
        if ($request->ajax()) {
            $payroll_groups = PayrollGroup::where('essentials_payroll_groups.business_id', $business_id)
                ...
                ->join('essentials_payroll_group_transactions as p', 'p.payroll_group_id', '=', 'essentials_payroll_groups.id')
            ...
            return Datatables::of($payroll_groups)
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        if (auth()->user()->can('essentials.delete_payroll')) {
                            $html .= '<li><a href="' . action('\Modules\Essentials\Http\Controllers\PayrollController@destroy', [$row->transaction_id]) . '" class="delete-payroll"><i class="fa fa-trash" aria-hidden="true"></i> ' . __("messages.delete") . '</a></li>';
                        }
                        if (auth()->user()->can('essentials.update_payroll')) {
                            $html .= '<li>
                ...
                ->editColumn(
                    ...
                    function ($row) {
                        ...
                        return (string) view('essentials::payroll.partials.payment_status', ['payment_status' => $payment_status, 'id' => $row->transaction_id]);
                    }
                )
    ...
    public function addPayment($id)
    {
        ...
        $role = $this->moduleUtil->getUserRoleName(Auth::user()->id);
        if ($role == 'Admin') {
            $accounts = Account::join('account_categories', 'account_categories.id', '=', 'accounts.account_category_id')
                ->where('account_categories.account_category', 'expence')
                ->where('accounts.is_closed', '0')
                ->select('accounts.id', 'accounts.account_number', 'accounts.name', 'accounts.business_id', 'account_categories.account_category_name')->get();
        } else {
            $accounts = Account::Join('user_has_accounts', 'user_has_accounts.account_id', 'accounts.id')
                ->join('business_locations', 'business_locations.business_id', '=', 'accounts.business_id')
                ->join('account_categories', 'account_categories.id', '=', 'accounts.account_category_id')
                ->where('account_categories.account_category', 'expence')
                ->where('accounts.is_closed', '0')
                ->where('user_has_accounts.user_id', Auth::user()->id)
                ->select(
                    'accounts.id',
                    'accounts.name',
    ...
    public function editPayment($id)
    {
        if (request()->ajax()) {
            ...
            return view('essentials::payroll.edit_pay_payroll_group')
